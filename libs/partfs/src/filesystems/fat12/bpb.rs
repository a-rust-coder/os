/// The BIOS Parameter Block (BPB) is the first sector of the volume. It indicates some important
/// infos about the volume. Actually, some fields are part of the Boot Sector (BS) and some others
/// to the BPB. These structures are thecnically different, but in practice it doesn't matter.
#[derive(Debug, Clone, Copy)]
pub struct BiosParameterBlock {
    /// Jump instruction to the boot code. Only the tow following forms are allowed:
    ///  - [ 0xEB, 0x??, 0x90 ]
    ///  - [ 0xE9, 0x??, 0x?? ]
    pub(in crate::filesystems::fat12) jmp_boot: [u8; 3],

    /// OEM Name identifier. Can be any desired value. Part of the BS.
    pub(in crate::filesystems::fat12) oem_name: [u8; 8],

    /// Count of bytes per sector. Must be one of 512, 1024, 2048, 4096.
    pub(in crate::filesystems::fat12) bytes_per_sector: u16,

    /// Count of sectors per cluster. Must be a power of 2.
    pub(in crate::filesystems::fat12) sectors_per_cluster: u8,
    ...
    pub(in crate::filesystems::fat12) reserved_sectors_count: u16,

    /// Number of FATs on the volume. 2 is recommended.
    pub(in crate::filesystems::fat12) number_of_fats: u8,
    ...
    /// `bytes_per_sector`.
    pub(in crate::filesystems::fat12) root_entries_count: u16,
    ...
    pub(in crate::filesystems::fat12) total_sectors_16: u16,

    /// Legal values are 0xF0, 0xF8, 0xF9, 0xFA, 0xFB, 0xFC, 0xFD, 0xFE, 0xFF.
    pub(in crate::filesystems::fat12) media: u8,

    /// Count of sectors occupied by one FAT.
    pub(in crate::filesystems::fat12) fat_size: u16,

    /// Irrelevant on volumes without geometry.
    pub(in crate::filesystems::fat12) sectors_per_track: u16,

    /// Irrelevant on volumes without geometry.
    pub(in crate::filesystems::fat12) number_of_heads: u16,

    /// Count of sectors preceding the partition that contains this FAT volume.
    pub(in crate::filesystems::fat12) hidden_sectors: u32,
